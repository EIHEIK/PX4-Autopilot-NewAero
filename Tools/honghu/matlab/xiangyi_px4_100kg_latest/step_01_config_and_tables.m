...
% 2026-07-31最终复飞：基线架次满足空中验收且未触发调参，因此
% 基线与紧随其后的重复架次共同构成“相同最终参数的连续两架次”。
if ~exist("px4UlogFiles", "var")
    px4UlogFiles = [ ...
        fullfile(repoRoot, "build", "px4_sitl_default", "rootfs", "log", ...
            "2026-07-30", "20_32_50.ulg"); ...
        fullfile(repoRoot, "build", "px4_sitl_default", "rootfs", "log", ...
            "2026-07-30", "20_58_05.ulg")];
end

if ~exist("outputRoot", "var")
    outputRoot = fullfile(repoRoot, "analysis_outputs", ...
        "honghu_v8_xiangyi_latest_20260731", "matlab");
end
...
assert(isfile(xiangyiCsv), "找不到翔仪CSV：" + xiangyiCsv);
assert(isfile(planFile), "找不到共同任务航线：" + planFile);
assert(all(isfile(px4UlogFiles)), ...
    "找不到最终PX4 ULog；请先完成两次100 kg复飞并更新px4UlogFiles。");
...
disp("100 kg FRD插值惯量 [Ixx Iyy Izz Ixy Ixz Iyz]：")
disp(inertia100FRD)
disp("PX4最终复飞ULog：")
disp(px4UlogFiles)
